...
const HRLayout = () => {
  ...
  // Limited menu items for HR Admin
  const menuItems = [
        ...
        >
          ...
          <ScrollArea className={cn("transition-all duration-300", sidebarCollapsed ? "h-[calc(100%-5rem)]" : "h-[calc(100%-5rem)]")}>
            <nav className={cn(
              ...
              sidebarCollapsed ? "p-2 space-y-1" : "p-4 space-y-1.5"
            )}>
              {menuItems.map((item) => {
                const Icon = item.icon;
                const hasCount = menuItemCounts[item.path] !== undefined && menuItemCounts[item.path] > 0;
                return (
                  <Link
                    key={item.path}
                    to={item.path}
